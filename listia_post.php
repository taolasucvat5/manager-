		...
		if(!$info["error"]){
			...
			switch ($_GET['is']) {
				...
				case 'auction_detail':
					...
					echo "<a href='".$auction_info["link_auction"]."'><h3 style='text-align:center;'>".$auction_info["title"]."</h3></a>";
					...
					';
					echo '<h4>Số Comment</h4>';
					foreach ($auction_info["all_comment"] as $comment) {
						foreach ($comment["comments"] as $key => $cmt) {
							if(contains($cmt["body"], "this comment has been hidden"))
								break;
							if($cmt["id"] != ""){
								echo "<div class='cmts ".(($cmt["is_child"] || $key > 0)?"indent":"")."'>";
								echo "<img width='35px' height='35px' src='".$cmt["avatar"]."'><a href='".$cmt["profile"]."' >".$cmt["username"]."</a><br />";
								echo "<p>".$cmt["body"]."</p>";
								echo "</div>";
							}else{
								echo "<div class='cmts ".(($cmt["is_child"] || $key > 0)?"indent":"")."'>";
								echo "<img width='35px' height='35px' src='".$cmt["avatar"]."'><a href='".$cmt["profile"]."' >".$cmt["username"]."</a><br />";
								echo '<form action="listia_post.php?action=chucnang&is=new_reply&style=web" method="post">
								<textarea rows="4" cols="50" name="body_reply"></textarea>
								<input type="hidden" name="authenticity_token" value="'.$auction_info["token"].'">
								<input type="hidden" name="action_link" value="'.$comment["reply"]["link"].'">
								<input type="hidden" name="id_reply" value="'.$comment["reply"]["id"].'">
					...
					echo '<h4>New Comment</h4>';
					if($auction_info["new_comment"]["check"]){
						echo '<form action="listia_post.php?action=chucnang&is=new_comment&style=web" method="post">
							<textarea rows="4" cols="50" name="body_comment"></textarea>
							<input type="hidden" name="authenticity_token" value="'.$auction_info["token"].'">
							<input type="hidden" name="action_link" value="'.$auction_info["new_comment"]["link"].'">
				...
				case 'new_comment':
					newComment($ch, array(
						"body_comment" => $_POST["body_comment"],
						"authenticity_token" => $_POST["authenticity_token"]
...
function newReply($ch, $data, $action_link){
	$postinfo = "utf8=%E2%9C%93&commit=Reply&reply%5Bvv%5D=beta&reply%5Bcomment%5D=".urlencode($data["body_reply"])."&reply%5Bcomment_id%5D=".urlencode($data["id_reply"]);
	curl_setopt($ch, CURLOPT_URL, $action_link);
	curl_setopt($ch, CURLOPT_RETURNTRANSFER, true);
	curl_setopt($ch, CURLOPT_POST, true);
	curl_setopt($ch, CURLOPT_FOLLOWLOCATION, 1); 
	curl_setopt($ch,CURLOPT_USERAGENT,'Mozilla/5.0 (Windows NT 10.0; Win64; x64; rv:56.0) Gecko/20100101 Firefox/56.0');
...
function newComment($ch, $data, $action_link){
	$postinfo = "utf8=%E2%9C%93&comment%5Bcomment%5D=".urlencode($data["body_comment"])."&commit=Comment";
	curl_setopt($ch, CURLOPT_URL, $action_link);
	curl_setopt($ch, CURLOPT_RETURNTRANSFER, true);
	curl_setopt($ch, CURLOPT_POST, true);
	curl_setopt($ch, CURLOPT_FOLLOWLOCATION, 1); 
...
function getAuctionFromNotify($ch, $list_id = array()){
	...
	try {
		...
		if($xpath->query("*//ul[@id = 'comment-list']/li")->length){
			$ul_comments = $xpath->query("*//ul[@id = 'comment-list']/li");
			$list_comment = array();

			foreach ($ul_comments as $comment){

				$temp = array();

				
				foreach ($comment->getElementsByTagName("div") as $div) {
					$div_comments = array();

					if( contains($div->getAttribute("class"), "comment-content")){
						$div_comments["id"] = $div->getAttribute("data-comment-id");
						$div_comments["avatar"] = $div->getElementsByTagName("img")[0]->getAttribute("src");

						foreach ($div->getElementsByTagName("a") as $tt) {
							if($tt->getAttribute("class") == "lt-user-login"){
								$div_comments["username"] = $tt->nodeValue;
								$div_comments["profile"] = "https://www.listia.com".$tt->getAttribute("href");

							}
						}
						$div_comments["body"] = getElementByClass($div, "div", "comment")->nodeValue;

						if($div->getAttribute("class") == "comment-content reply")
							$div_comments["is_child"] = true;
						else
							$div_comments["is_child"] = false;

						$temp["comments"][] = $div_comments;
					}

				}
				

				if($form_reply = getElementByClass($comment,"div" , "comment-content reply reply-form")){

					$temp["reply"]["check"] = true;
					$temp["reply"]["link"] = "https://www.listia.com".getElementByClass($form_reply,"form","reply-form")->getAttribute("action");
					foreach ($form_reply->getElementsByTagName("input") as $ip) {
						if($ip->getAttribute("name") == "reply[comment_id]"){
							$temp["reply"]["id"] = $ip->getAttribute("value");
							break;
						}
					}
				}else{
			...
			$result["all_comment"] = $list_comment;
		}

		if($form_comment = $dom->getElementById("comment-form")){

				$result["new_comment"]["check"] = true;
				$result["new_comment"]["link"] = "https://listia.com".$dom->getElementById("comment-form")->getAttribute("action");
					
		}else{

				$result["new_comment"]["check"] = false;
		}

		$result["token"] = $xpath->query("//meta[@name = 'csrf-token']")[0]->getAttribute("content");

		if($xpath->query("//meta[@property = 'og:url']")->length){
			$link_auction = $xpath->query("//meta[@property = 'og:url']")[0]->getAttribute("content");
			$result["link_auction"] = $link_auction ;
		}

		if($xpath->query("//div[@class = 'seller']/a[@class = 'lt-user-login']")){
			$result["seller"] = $xpath->query("//div[@class = 'seller']//a[@class = 'lt-user-login']")[0]->nodeValue;
		}
